...
int main() {
	auto vec = std::vector<int>{5, -3, 2, 7, 11};
...
int main() {
	auto vec1 = std::vector<int>{5, -3, 2, 7, 11};
	auto vec2 = std::vector<int>{0, 1, 2, 3};
...
Careful readers may have noticed that the naming-requirements for functions did not
explicitly state that the name has to be unique. That is because it doesn't have to.
Functions are identified not only by their names, but also by their arguments.
If the arguments of two functions differ in number or type,
it is valid for them to share a name.
...
current version works for `int`s, we might be interested in a related but not identical function
for strings: Find the smallest character.

To keep this task simple, we'll restrict ourselves to the
lowercase-characters of the English alphabet and
...
While the new code may still be somewhat redundant,
it shows that there is no problem with creating two
different functions that share a name and an abstract
behavior (finding the smallest element in some kind
of list) but differ in implementation.
